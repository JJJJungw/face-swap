...
| 부위 경계 게이트 edge loss (`--w-edge-part`) | ❌ | 계단 비율 3.0%→**2.0%**, 경계 표본 5,232→5,804. **선은 더 그렸는데 더 부드럽게 그렸다.** Sobel L1 은 폭에 눈이 멀어서, 위치를 좁혀도 넓은 경사로 만족시킨다 | [training-history](training-history.md#결핍은-화풍이-아니라-일반화였다-2026-08-07) |
| 소프트 램프 네거티브 (`--w-adv-soft`) | ⏸ | 블러한 타겟을 D 에 '가짜'로. 전이폭 10.13→9.26, 부위 선밀도 5개 개선(턱 0.35→0.48). **그러나 육안 "선이 지저분해짐" + val L1 0.1411→0.1421. 블라인드 A/B 미실시 = 순이득 미확정** | 〃 |
| 각도(포즈) 격차가 원인이라는 가설 | ❌ | 분포는 실제로 다르다(코퍼스 3/4 24% vs 실전 54.8%). **그러나 코퍼스 정면 0.91 / 3-4 0.92 로 차이가 없다.** 원인은 각도가 아니라 입력 도메인 | 〃 |
