...
### 현재 실측 (2026-08-06, L40S · swap10 720x1280 · 크롭 530px 근처)

```
DONE 77.7s / video 37.0s = 2.10x realtime
프레임당 ms: 검출 8.2 | 합성 59.3(그중 GAN 6.4, CPU합성 52.8) | 인코딩write 1.8
```

**병목은 CPU 합성 52.8ms — 전체 69.4ms 의 76%다.** GPU 쪽은 이미 충분히 빠르다
(GAN 6.4ms, 검출 8.2ms, 인코딩 1.8ms).

지금 켜둔 옵션이 전부 CPU 픽셀 연산이고, 크롭 하나마다 돈다.

| 단계 | 하는 일 |
|---|---|
| `--color-mode chroma` | BGR↔Lab 왕복 + 크로마 채널 블러 |
| `--luma-match 0.7` | 저주파 휘도 매칭 (`lowpass` 2회) |
| `--darken 1.8` | luma 변환 + 블러 2회 |
| 마스크 페더 | `lowpass` |

**비용은 크롭 면적에 비례한다.** 이 영상은 크롭 한 변이 530px 근처라 작은 얼굴 영상보다
2~3배 비싸다. 다음 최적화 대상은 이 경로를 GPU 로 옮기는 것.

> **⚠️ 위 1.02× 와 이 2.10× 는 같은 조건이 아니다.** 1.02× 는 옵션이 적던 시절 값이고,
> 이후 `--color-mode chroma` · `--luma-match` · `--darken` 이 순서대로 추가됐다.
> **옵션을 바꾸면 속도 수치는 이전과 비교하지 않는다.**
