...
| ④ 학생 학습 | 페어로 경량 GAN 지도학습(자체 가중치) **+ 신원 억제 손실** | AnimeGANv2 아키텍처 | MIT(코드) | 🔜 |
| ⑤ 런타임 교체 | 학생 가중치를 런타임 셸에 삽입 | 기존 ONNX→TRT 파이프라인 | Apache | 🔜 |

> ⚠️ **비식별 보장은 스타일화만으론 부족.** 스타일화해도 스타일-강건 인식기는 신원을 재식별함(재식별 0.744, [research-report](docs/research-report.md)). 따라서 ④ 학생 학습 목표에 **신원 억제 손실**(원본 얼굴 임베딩 vs 출력 임베딩의 코사인을 낮춤)을 반드시 포함해 "예쁘게 바뀐 같은 사람"이 아니라 진짜 비식별이 되게 한다.
